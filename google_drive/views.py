...
def notify(request):
	...
	if decrypted_token == expected_token:
		...
		try:
			...
			# check the database for known files for this project:
			# get datasources from db:
			datasources = project.datasource_set.all()
			datasource_paths = [x.filepath for x in datasources]

			uploads = [x.name for x in bucket_obj.list_blobs() if x.name.startswith(settings.UPLOAD_PREFIX)]

			#check the names of the files to ensure they meet our standards:
			passing_files_in_bucket = set()
			for f in uploads:
				try:
					helpers.determine_filetype(f)
					passing_files_in_bucket.add(f)
				except helpers.UndeterminedFiletypeException as ex:
					pass

			# now have 'good' files that are in the bucket and a set that are in the database.  update the database:
			datasource_set = set(datasource_paths)
			diff_set = passing_files_in_bucket.difference(datasource_set)
			for d in diff_set:
				helpers.add_datasource_to_database(project, d)			
...
def launch_custom_instance(compute, config_params):
    ...
    callback_url = '%s/%s' % (settings.HOST, config_params['callback_url'])
    ...
    config = {
	    ...
	    {'key':'token', 'value':config_params['token']},
	    {'key':'enc_key', 'value':config_params['enc_key']},
          ]
        }
    }
