...
def create_inputs_json(sample_name, first, second, genome, config):
    '''
    ...
    #                              sample_name + ".inputs.json")
    rg_groups = "\\t".join(["@RG",
                            "ID:" + sample_name + ".filler_ID",
                            "PU:" + sample_name + ".filler_PU.1",
                            "LB:" + sample_name + ".filler_LB",
                            "PL:assumed_ILLUMINA",
                            "SM:" + sample_name])
    inputs_filename = '.'.join([sample_name, "inputs.json"])
    d = {"BUCKET_INJECTION": config.get('default_templates',
                                        'reference_bucket'),
         "FIRST_INJECTION": first,
         "SECOND_INJECTION": second,
         "INPUT_BASENAME_INJECTION": sample_name,
         "INPUT_RG_GROUPS": rg_groups,
...
def start_analysis(project_pk):
    ...
    for key, ds_list in sample_mapping.items():
        ...
        fastqs = sorted(["gs://" + os.path.join(project.bucket, ds.filepath)
                         for ds in ds_list])
        if len(fastqs) == 1:
            first = fastqs[0]
            second = ""
            ...
            first, second = fastqs[:2]
        #first = "gs://" + os.path.join(project.bucket, first.filepath)
        #if len(fastqs) > 1:
        #    second = "gs://" + os.path.join(project.bucket, second.filepath)
        #else:
        #    second = ""
        inputs_json = create_inputs_json(sample_name, first, second, "hg19",
                                         config)
        ...
        codes.append((code_key,
                      sample_name,
                      '.'.join([sample_name, 'sorted.bqsr.bam']),
    ...
    project.status_message = "Running variant calling pipeline"
    project.save()
    code_map = '|'.join(['`'.join(c) for c in codes])
    print "DEBUG:", code_map
    tasks.check_completion.delay(project_pk, code_map, bucket_name)
