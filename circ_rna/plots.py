...
def make_plot(reads, ordered_exons, junction_to_exons_map, primary_junction_id, output_directory):
...
def plot_circ_rna(high_quality_junctions, reads_file, gtf_file, output_directory):
	"""
	high_quality_junctions is an array of junction IDs
	reads_file has the individual reads which we use to plot
	gtf_file is a path to a GTF file
	output_directory is where to write the figures
	"""

	# load the table of reads:
	reads = pd.read_table(reads_file)
	reads = reads.ix[reads['junction'].isin(high_quality_junctions)]
	...
	for j in high_quality_junctions:
		...
		if ordered_exons and junction_to_exons_map:
			make_plot(reads_subset, ordered_exons, junction_to_exons_map, j, output_directory)
		else:
			with open(os.path.join(output_directory, 'notes.txt'), 'wa') as fout:
